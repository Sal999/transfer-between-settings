#' Add ISS Trauma Severity Indicator
#'
#' Adds a variable to the data that indicates if a patient has an Injury
#' Severity Score (ISS) above or equal to a certain cutoff.
...
#' @param cutoff Numeric vector of length 1. The ISS severity cutoff. Has to be
#'     between 3 and 75. Defaults to 15.
...
AddISSTraumaSeverityIndicator <- function(study.sample,
  ...
  if (!is.numeric(cutoff) | !IsLength1(cutoff) | cutoff < 3 | cutoff > 75)
    stop ("cutoff has to be a numeric vector of length 1 between 3 and 75")
  ...
  ## Get variable data
  variable.data <- as.integer(study.sample[, severity.variable.name])
  ## Check if there are entries greater than the cutoff
  indicator.data <- as.factor(ifelse(variable.data > cutoff,2,1))
